...
export function MobileNav({ open, onClose, onLogout }: MobileNavProps) {
  ...
  return (
    <div className="md:hidden fixed inset-0 bg-slate-900/30 backdrop-blur-sm z-40" onClick={onClose}>
      <div className="bg-white w-64 h-full flex flex-col p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-lg font-bold mb-6 text-slate-900">Menu</h2>
        ...
        <div className="flex flex-col gap-1.5 pt-4 border-t border-slate-200">
          ...
          <button
            ...
            className="px-4 py-3 rounded-lg font-semibold text-left text-slate-600 hover:bg-slate-100 transition-colors"
          >
            Logout
          </button>
        </div>
